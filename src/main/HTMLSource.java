...
public class HTMLSource {
	...
	public void setarValores(Integer numero, String nome, Blusa blusa) {
		stringsHTML.append("<tr>").append("<td>").append(numero).append("</td>");
		stringsHTML.append("<td>");
		if (isFeminino()) {
			stringsHTML.append("<font color='#FF00FF'><strong>").append(nome).append("</strong></font>")
					.append("</td>");
		} else {
	...
	public void gerarDados(Integer numero, String[] textoDoArquivoSeparado, int posicaoArray)
	...
	public String regexNome(String nome) {
		return nome.replace("\"", "").replace("-", "").replace(" ", "").replace("no", "na");
	}
	...
	public String gerarListaInscritos() {
	...
	public String getNome() {
		return this.textoDoArquivoSeparado[COLUNA_NOME].replace("\"", "").concat(" ")
				+ this.textoDoArquivoSeparado[COLUNA_SOBRE_NOME].replace("\"", "");
	}
	...
	private int totalMasculinas() {
		return (BlusaMasculinaP.getTotal() + BlusaMasculinaM.getTotal() + BlusaMasculinaG.getTotal()
				+ BlusaMasculinaGG.getTotal() + BlusaMasculinaEXG.getTotal());
	}

	private int totalFemininas() {
		return (BlusaFemininaP.getTotal() + BlusaFemininaM.getTotal() + BlusaFemininaG.getTotal()
